...
# ============================================
# ПРАВИЛЬНЫЙ ПУТЬ К JSON - ПОСЛЕДНЯЯ ВЕРСИЯ
# ============================================

# Логируем текущую директорию для понимания структуры
current_dir = Path(__file__).parent
logging.info(f"📁 Текущая папка бота: {current_dir}")
logging.info(f"📁 Родительская папка: {current_dir.parent}")

# На основе логов видно, что проект в корне /
# А папка data должна быть в /data
DATA_DIR = Path("/data")
logging.info(f"📁 Путь к данным: {DATA_DIR}")

# Проверяем существование папки
if DATA_DIR.exists():
    logging.info(f"✅ Папка /data существует")
    ...
    if files:
        ...
        logging.warning(f"⚠️ В папке /data нет JSON файлов")
else:
    logging.error(f"❌ Папка /data НЕ СУЩЕСТВУЕТ!")
    # Пробуем создать
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        logging.info(f"✅ Папка /data создана")
    except:
        logging.error(f"❌ Не удалось создать папку /data")

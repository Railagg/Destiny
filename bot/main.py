...
# ============================================
# КОПИРУЕМ JSON ИЗ РЕПОЗИТОРИЯ В /data
# ============================================

# Откуда копируем — папка data рядом с bot/
source_dir = Path(__file__).parent.parent / "data"
# Куда копируем — в корневую /data
target_dir = Path("/data")

logging.info(f"📂 Source (репозиторий): {source_dir}")
logging.info(f"📂 Target (куда копируем): {target_dir}")

# Создаём целевую папку, если её нет
target_dir.mkdir(parents=True, exist_ok=True)
logging.info(f"✅ Папка {target_dir} создана или уже существует")

# Копируем все JSON из репозитория в /data
if source_dir.exists():
    json_files = list(source_dir.glob("*.json"))
    logging.info(f"📄 Найдено JSON в репозитории: {len(json_files)}")
    
    for json_file in json_files:
        shutil.copy(json_file, target_dir / json_file.name)
        logging.info(f"✅ Скопирован {json_file.name}")
else:
    logging.error(f"❌ Папка {source_dir} не найдена в репозитории!")

# Проверяем, что скопировалось
copied_files = list(target_dir.glob("*.json"))
logging.info(f"📄 JSON в {target_dir} после копирования: {len(copied_files)}")

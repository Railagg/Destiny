...
async function loadGameData() {
    try {
        ...
        if (response.ok) {
            ...
            console.log(`   ⚔️ Врагов: ${Object.keys(window.enemiesData).length}`);
            console.log(`   📦 Предметов: ${Object.keys(window.itemsData).length}`);
...
function loadPage(page) {
    ...
    if (page === 'game') {
        ...
        // Если есть страница крафта
        if (typeof showCraftingMenu === 'function') {
            showCraftingMenu();
            return;
        }
    } else if (page === 'quests') {
        if (typeof showQuestsMenu === 'function') {
            showQuestsMenu();
            return;
        }
    } else if (page === 'house') {
        if (typeof showHouseMenu === 'function') {
            showHouseMenu();
            return;
        }
    } else {
...
async function doAction(type, target) {
    ...
    // ===== БОЙ =====
        ...
        if (typeof startCombat === 'function') {
            ...
            showNotification(`⚔️ Бой с ${target} (система боя загружается...)`);
        }
    }
    ...
    // ===== ОТКРЫТЬ СУНДУК =====
        ...
        setTimeout(() => {
            ...
            const items = ['health_potion', 'iron_ingot', 'gold_ingot', 'crystal'];
    ...
    // ===== КРАФТ =====
    else if (type === 'craft_armor' || type === 'craft_weapon' || type === 'craft_tools') {
        if (typeof showCraftingMenu === 'function') {
            showCraftingMenu(type.replace('craft_', ''));
        } else {
            showNotification('🛠️ Система крафта загружается...');
        }
    }
    else if (type === 'smelt_ore') {
        if (typeof smeltOre === 'function') {
            smeltOre();
        }
    }
    else if (type === 'craft_glass') {
        if (typeof craftGlass === 'function') {
            craftGlass();
        }
    }
    ...
    // ===== ДОМИК =====
    else if (type === 'build_house' || type === 'upgrade_house' || type === 'open_storage') {
        if (typeof showHouseMenu === 'function') {
            showHouseMenu();
        } else {
            loadPage('house');
    ...
    else if (type === 'rest_in_house') {
        if (typeof restInHouse === 'function') {
            restInHouse();
        }
    }
    ...
    // ===== РАЗГОВОР =====
        ...
        // Особые случаи
        if (target === 'tavern_quest') {
            if (typeof acceptQuest === 'function') {
                acceptQuest('kill_wolf');
            }
        } else if (target === 'goblin_secret') {
            showNotification('🤫 Гоблин шепчет: "В руинах есть тайник..."');
        }
    }
    
    // ===== ТЕЛЕПОРТ =====
    else if (type === 'teleport_menu') {
        showNotification('✨ Телепорт пока в разработке');
    }
    ...
    // ===== РЫБАЛКА =====
        ...
        setTimeout(() => {
            if (Math.random() > 0.3) {
                const fish = ['fish', 'goldfish'][Math.floor(Math.random() * 2)];
                if (!playerData.inventory) playerData.inventory = [];
                ...
                if (typeof updateQuestProgress === 'function') {
                    updateQuestProgress('collect', 'fish');
                }
            } else {
    ...
    // ===== ШАХТА =====
        ...
        setTimeout(() => {
            const ores = ['copper_ore', 'iron_ore', 'coal'];
            ...
            // Обновляем прогресс квеста
            if (typeof updateQuestProgress === 'function') {
                updateQuestProgress('collect', ore);
            }
        }, 1500);
    }
...
// Загружаем все модули после основного скрипта
setTimeout(() => {
    console.log('🔄 Загрузка дополнительных модулей...');
    
    // Проверяем наличие функций из других файлов
    const modules = {
        combat: typeof startCombat === 'function',
        craft: typeof showCraftingMenu === 'function',
        quests: typeof showQuestsMenu === 'function',
        house: typeof showHouseMenu === 'function'
    };
    
    console.log('📊 Статус модулей:', modules);
    
    if (!modules.combat) console.warn('⚠️ Модуль боя не загружен');
    if (!modules.craft) console.warn('⚠️ Модуль крафта не загружен');
    if (!modules.quests) console.warn('⚠️ Модуль квестов не загружен');
    if (!modules.house) console.warn('⚠️ Модуль домика не загружен');
